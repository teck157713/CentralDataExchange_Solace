var QueueProducer = function (queueName) {
    ...
    producer.sendgovdata = function () {
      govdataon = setInterval(producer.sendGovText, 10000);
      producer.log('Publishing, the interval has been started.');
    }

    producer.stopgovdata = function () {
      clearInterval(govdataon);
      producer.log('Publishing has been stopped.');
    }

    producer.sendGovText = function () {
        if (producer.session !== null) {
            processgov(function(resdict){
              producer.sendviaTopics(resdict);
            });
        } else {
            producer.log('Cannot send messages because not connected to Solace message router.');
        }
    ...
    producer.sendviaTopics = function (result) {
        ...
        function setTopic(x){
          if (x.length == 0){
            ...
            message.setDestination(solace.SolclientFactory.createTopicDestination('tutorial/queue/' + x.pop()));
            ...
            try {
                ...
                producer.log('Message #' + messageText + ' sent to queue "' + producer.queueName + '"' + JSON.stringify(correlationKey));
            } catch (error) {
                producer.log(error.toString());
            }
            return setTopic(x);
