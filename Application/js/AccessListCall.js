...
function AccessInnerCall(username, type, value = '', counter = 0, filter = '') {
    ...
    const AssignActionParams = type =>
        type == "POST" ? (counter == 1 ? (insertedtype = "POST", insertedvalue = '{"publishExceptionTopic": "' + value + '", "topicSyntax" : "smf" }'): undefined)
        : (counter == 2 ? (insertedtype = "POST", insertedvalue = '{"subscribeExceptionTopic": "' + value + '", "topicSyntax" : "smf" }'):undefined);
    AssignActionParams(type);

    // Assign the right URI based on subsequent API calls
    const AssignUriBase = (counter, filter) =>
        counter == 0 ? (!filter ? "http://localhost:8080/SEMP/v2/config/msgVpns/default/clientUsernames/" + username : "http://localhost:8080/SEMP/v2/config/msgVpns/default/clientUsernames")
        : counter == 1 ? ("http://localhost:8080/SEMP/v2/config/msgVpns/default/aclProfiles/" + username + "/publishExceptions")
        : counter == 2 ? ("http://localhost:8080/SEMP/v2/config/msgVpns/default/aclProfiles/" + username + "/subscribeExceptions")
        : counter == 3 ? ("http://localhost:8080/SEMP/v2/config/msgVpns/default/aclProfiles") : undefined;
    UriSEMP = AssignUriBase(counter, filter);
    ...
    // IF the API calls failed at any process, return the error result.
