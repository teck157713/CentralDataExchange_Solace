...
for /f "usebackq tokens=1-5 delims=," %%a in ("RainData.csv") do (
      set "message="id":"%%a","lat":"%%b","long":"%%c","value":"%%d","timestamp":"%%e""
      echo !message!>txt.txt
      sdkperf_c -cip=tcp://mr8ksiwsp23vv.messaging.solace.cloud:20448 -cu=solace-cloud-client@msgvpn-jfgwkeg1ahl -cp=croo89sd2pkbmk49a325m21q1v -stl=NEA/1/rain_data/raw/%%b/%%c -ptl=NEA/1/rain_data/raw/%%b/%%c -mn=1 -md -pal=txt.txt )
